...
contract Monsweeper is Ownable {
    ...
    function submitMovesAndCashOut(bytes32 gameId, uint8[] calldata clickedTiles) external {
        ...
        // Generate exactly 9 unique bomb positions
        while (bombsPlaced < NUM_BOMBS && attempts < 100) {
            bytes32 hash = keccak256(abi.encode(currentSeed, bombsPlaced));
            uint8 bombPos = uint8(uint(hash) % BOARD_SIZE);
            
            // Only place bomb if position is not already a bomb
            if (!isBomb[bombPos]) {
                isBomb[bombPos] = true;
                bombsPlaced++;
            }
